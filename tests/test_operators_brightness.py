"""Unit tests for :mod:`src.augmentation.operators.brightness`.

Covers ``BrightnessOperator``, including construction-time validation,
deterministic parameter sampling, brightness increase/decrease
...
def mid_gray_image() -> np.ndarray:
    """A flat mid-gray synthetic image, useful for exact-factor assertions."""
...
class TestInitialization:
    """Tests for BrightnessOperator construction and exposed properties."""
    ...
    def test_custom_construction(self) -> None:
        ...
        op = BrightnessOperator(
            ...
            enabled=False,
            random_state=7,
            operator_name="custom_brightness",
        )

        assert op.min_factor == 0.5
        assert op.max_factor == 1.8
        assert op.probability == 0.9
        assert op.enabled is False
        assert op.operator_name == "custom_brightness"

    def test_equal_bounds_allowed(self) -> None:
        """A degenerate range where min_factor == max_factor is accepted."""
        op = BrightnessOperator(min_factor=1.1, max_factor=1.1, random_state=0)
    ...
    def test_exposes_random_state_generator(self) -> None:
        ...
        op = BrightnessOperator(random_state=1)
...
class TestInvalidConfiguration:
    ...
    def test_non_positive_min_factor_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="min_factor"):
            BrightnessOperator(min_factor=0.0)
        with pytest.raises(ValueError, match="min_factor"):
            BrightnessOperator(min_factor=-0.5)
...
class TestDeterministicRandomness:
    ...
    def test_same_seed_produces_identical_output(self, image: np.ndarray) -> None:
        ...
        op1 = BrightnessOperator(random_state=123)
        op2 = BrightnessOperator(random_state=123)
    ...
    def test_same_seed_produces_identical_gate_decisions(
        ...
        op1 = BrightnessOperator(probability=0.5, random_state=99)
        op2 = BrightnessOperator(probability=0.5, random_state=99)
    ...
    def test_different_seeds_produce_different_factors(
        ...
        op1 = BrightnessOperator(random_state=1)
        op2 = BrightnessOperator(random_state=2)
    ...
    def test_factor_within_configured_range(self, image: np.ndarray) -> None:
        ...
        op = BrightnessOperator(min_factor=0.6, max_factor=1.4, random_state=5)
...
class TestBrightnessAdjustment:
    """Tests for the directional effect of brightness adjustment."""

    def test_brightness_increase(self, mid_gray_image: np.ndarray) -> None:
        """A factor above 1.0 increases pixel intensity proportionally."""
        op = BrightnessOperator(min_factor=1.3, max_factor=1.3, random_state=0)
        result, params = op._apply(mid_gray_image)

        assert params["factor"] == pytest.approx(1.3)
        assert result.mean() > mid_gray_image.mean()
        np.testing.assert_array_equal(result, np.full_like(mid_gray_image, 130))

    def test_brightness_decrease(self, mid_gray_image: np.ndarray) -> None:
        """A factor below 1.0 decreases pixel intensity proportionally."""
        op = BrightnessOperator(min_factor=0.7, max_factor=0.7, random_state=0)
        result, params = op._apply(mid_gray_image)

        assert params["factor"] == pytest.approx(0.7)
        assert result.mean() < mid_gray_image.mean()
        np.testing.assert_array_equal(result, np.full_like(mid_gray_image, 70))

    def test_unchanged_factor_is_identity(self, image: np.ndarray) -> None:
        """A factor of exactly 1.0 leaves the image unchanged."""
        op = BrightnessOperator(min_factor=1.0, max_factor=1.0, random_state=0)
...
class TestClipping:
    ...
    def test_clipping_at_255(self, bright_image: np.ndarray) -> None:
        """A large brightness factor clips pixel values at 255, not overflow."""
        op = BrightnessOperator(min_factor=3.0, max_factor=3.0, random_state=0)
    ...
    def test_clipping_never_produces_values_above_255_or_below_0(
        self, image: np.ndarray
    ) -> None:
        """Output pixel values always remain within [0, 255] regardless of factor."""
        op = BrightnessOperator(min_factor=0.1, max_factor=5.0, random_state=3)
...
class TestGating:
    ...
    def test_probability_zero_never_applies(self, image: np.ndarray) -> None:
        ...
        op = BrightnessOperator(probability=0.0, random_state=123)
    ...
    def test_enabled_false_never_applies(self, image: np.ndarray) -> None:
        ...
        op = BrightnessOperator(probability=1.0, enabled=False, random_state=123)
    ...
    def test_probability_one_always_applies(self, image: np.ndarray) -> None:
        ...
        op = BrightnessOperator(probability=1.0, random_state=123)
...
class TestAugmentationResultContents:
    ...
    def test_successful_result_contents(self, image: np.ndarray) -> None:
        ...
        op = BrightnessOperator(probability=1.0, random_state=1)
        result = op.apply(image)

        assert isinstance(result, AugmentationResult)
        assert result.operator_name == "brightness"
        assert result.applied is True
        assert result.success is True
    ...
    def test_skipped_result_contents(self, image: np.ndarray) -> None:
        ...
        op = BrightnessOperator(enabled=False, random_state=1)
        result = op.apply(image)
    ...
    def test_custom_operator_name_is_reflected_in_result(
        ...
        op = BrightnessOperator(
            probability=1.0, random_state=1, operator_name="my_brightness"
        )
        result = op.apply(image)

        assert result.operator_name == "my_brightness"
...
class TestShapeAndDtypePreservation:
    ...
    def test_output_shape_preserved(self, image: np.ndarray) -> None:
        ...
        op = BrightnessOperator(probability=1.0, random_state=2)
        result = op.apply(image)
    ...
    def test_various_shapes_preserved(self, height: int, width: int) -> None:
        ...
        op = BrightnessOperator(probability=1.0, random_state=0)
...
class TestRegistryCompatibility:
    """Tests confirming BrightnessOperator integrates with OperatorRegistry."""

    def test_register_and_retrieve(self) -> None:
        """BrightnessOperator can be registered and retrieved by name."""
        registry = OperatorRegistry()

        registry.register("brightness", BrightnessOperator)

        assert registry.get("brightness") is BrightnessOperator
    ...
    def test_instantiate_from_registry(self, image: np.ndarray) -> None:
        ...
        operator_cls = registry.get("brightness")
    ...
    def test_is_subclass_of_base_augmentation(self) -> None:
        """BrightnessOperator is a proper subclass of BaseAugmentation."""
        from src.augmentation.operators.base import BaseAugmentation

        assert issubclass(BrightnessOperator, BaseAugmentation)