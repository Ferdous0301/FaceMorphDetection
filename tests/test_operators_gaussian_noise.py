"""Unit tests for :mod:`src.augmentation.operators.gaussian_noise`.

Covers ``GaussianNoiseOperator``, including construction-time
validation, deterministic parameter and noise sampling, the effect of
zero versus positive sigma, clipping, probability/enabled gating, the
contents of the returned :class:`AugmentationResult`, and
compatibility with :class:`OperatorRegistry`.

Only synthetic NumPy arrays are used as test images; no real images
are read from disk.
"""

from __future__ import annotations

import numpy as np
import pytest

from src.augmentation.operators.base import (
    AugmentationResult,
    BaseAugmentation,
    OperatorRegistry,
)
from src.augmentation.operators.gaussian_noise import GaussianNoiseOperator


# ---------------------------------------------------------------------------
# Fixtures
# ---------------------------------------------------------------------------


@pytest.fixture
def image() -> np.ndarray:
    """A deterministic, non-trivial synthetic RGB image."""
    rng = np.random.default_rng(42)
    return rng.integers(0, 256, size=(32, 48, 3), dtype=np.uint8)


@pytest.fixture
def bright_image() -> np.ndarray:
    """A near-white synthetic image, useful for exercising clipping at 255."""
    return np.full((16, 16, 3), 250, dtype=np.uint8)


@pytest.fixture
def dark_image() -> np.ndarray:
    """A near-black synthetic image, useful for exercising clipping at 0."""
    return np.full((16, 16, 3), 5, dtype=np.uint8)


# ---------------------------------------------------------------------------
# Initialization
# ---------------------------------------------------------------------------


class TestInitialization:
    """Tests for GaussianNoiseOperator construction and exposed properties."""

    def test_default_construction(self) -> None:
        """Default construction exposes the documented default sigma range."""
        op = GaussianNoiseOperator()

        assert op.min_sigma == 2.0
        assert op.max_sigma == 15.0
        assert op.operator_name == "gaussian_noise"
        assert op.probability == 0.5
        assert op.enabled is True

    def test_custom_construction(self) -> None:
        """Custom constructor arguments are stored and exposed correctly."""
        op = GaussianNoiseOperator(
            min_sigma=1.0,
            max_sigma=25.0,
            probability=0.9,
            enabled=False,
            random_state=7,
            operator_name="custom_gaussian_noise",
        )

        assert op.min_sigma == 1.0
        assert op.max_sigma == 25.0
        assert op.probability == 0.9
        assert op.enabled is False
        assert op.operator_name == "custom_gaussian_noise"

    def test_equal_bounds_allowed(self) -> None:
        """A degenerate range where min_sigma == max_sigma is accepted."""
        op = GaussianNoiseOperator(min_sigma=5.0, max_sigma=5.0, random_state=0)

        assert op.min_sigma == op.max_sigma == 5.0

    def test_zero_sigma_bounds_allowed(self) -> None:
        """A sigma range of exactly (0.0, 0.0) is a valid configuration."""
        op = GaussianNoiseOperator(min_sigma=0.0, max_sigma=0.0, random_state=0)

        assert op.min_sigma == 0.0
        assert op.max_sigma == 0.0

    def test_exposes_random_state_generator(self) -> None:
        """The operator exposes a numpy.random.Generator via random_state."""
        op = GaussianNoiseOperator(random_state=1)

        assert isinstance(op.random_state, np.random.Generator)

    def test_is_subclass_of_base_augmentation(self) -> None:
        """GaussianNoiseOperator is a proper subclass of BaseAugmentation."""
        assert issubclass(GaussianNoiseOperator, BaseAugmentation)


# ---------------------------------------------------------------------------
# Invalid configuration
# ---------------------------------------------------------------------------


class TestInvalidConfiguration:
    """Tests for construction-time validation failures."""

    def test_negative_min_sigma_raises(self) -> None:
        """A negative min_sigma raises ValueError."""
        with pytest.raises(ValueError, match="min_sigma"):
            GaussianNoiseOperator(min_sigma=-0.1)
        with pytest.raises(ValueError, match="min_sigma"):
            GaussianNoiseOperator(min_sigma=-10.0)

    def test_negative_max_sigma_raises(self) -> None:
        """A negative max_sigma raises ValueError."""
        with pytest.raises(ValueError, match="max_sigma"):
            GaussianNoiseOperator(max_sigma=-0.1)
        with pytest.raises(ValueError, match="max_sigma"):
            GaussianNoiseOperator(max_sigma=-5.0)

    def test_min_greater_than_max_raises(self) -> None:
        """min_sigma greater than max_sigma raises ValueError."""
        with pytest.raises(ValueError, match="min_sigma"):
            GaussianNoiseOperator(min_sigma=20.0, max_sigma=5.0)

    def test_invalid_probability_raises(self) -> None:
        """An out-of-range probability raises an error via BaseAugmentation."""
        with pytest.raises(Exception):
            GaussianNoiseOperator(probability=1.5)


# ---------------------------------------------------------------------------
# Deterministic randomness
# ---------------------------------------------------------------------------


class TestDeterministicRandomness:
    """Tests for deterministic, seed-driven parameter and noise sampling."""

    def test_same_seed_produces_identical_output(self, image: np.ndarray) -> None:
        """Two operators with the same random_state produce identical output."""
        op1 = GaussianNoiseOperator(random_state=123)
        op2 = GaussianNoiseOperator(random_state=123)

        result1, params1 = op1._apply(image)
        result2, params2 = op2._apply(image)

        assert params1["sigma"] == params2["sigma"]
        np.testing.assert_array_equal(result1, result2)

    def test_noise_is_reproducible_with_same_random_state(
        self, image: np.ndarray
    ) -> None:
        """Repeated apply() calls on identically-seeded operators stay in lockstep."""
        op1 = GaussianNoiseOperator(min_sigma=5.0, max_sigma=5.0, random_state=55)
        op2 = GaussianNoiseOperator(min_sigma=5.0, max_sigma=5.0, random_state=55)

        for _ in range(5):
            result1, params1 = op1._apply(image)
            result2, params2 = op2._apply(image)

            assert params1 == params2
            np.testing.assert_array_equal(result1, result2)

    def test_same_seed_produces_identical_gate_decisions(
        self, image: np.ndarray
    ) -> None:
        """Two operators with the same random_state make identical gate decisions."""
        op1 = GaussianNoiseOperator(probability=0.5, random_state=99)
        op2 = GaussianNoiseOperator(probability=0.5, random_state=99)

        results1 = [op1.apply(image) for _ in range(10)]
        results2 = [op2.apply(image) for _ in range(10)]

        for result1, result2 in zip(results1, results2):
            assert result1.applied == result2.applied

    def test_different_seeds_produce_different_outputs(
        self, image: np.ndarray
    ) -> None:
        """Different random_state values yield different sampled noise."""
        op1 = GaussianNoiseOperator(min_sigma=10.0, max_sigma=10.0, random_state=1)
        op2 = GaussianNoiseOperator(min_sigma=10.0, max_sigma=10.0, random_state=2)

        result1, _ = op1._apply(image)
        result2, _ = op2._apply(image)

        assert not np.array_equal(result1, result2)

    def test_sigma_within_configured_range(self, image: np.ndarray) -> None:
        """The sampled sigma always falls within [min_sigma, max_sigma]."""
        op = GaussianNoiseOperator(min_sigma=3.0, max_sigma=12.0, random_state=5)

        for _ in range(25):
            _, params = op._apply(image)
            assert 3.0 <= params["sigma"] <= 12.0


# ---------------------------------------------------------------------------
# Zero vs. positive sigma behaviour
# ---------------------------------------------------------------------------


class TestSigmaEffect:
    """Tests for the effect of sigma magnitude on the output image."""

    def test_zero_sigma_produces_identical_image(self, image: np.ndarray) -> None:
        """A sigma of exactly 0.0 leaves the image completely unchanged."""
        op = GaussianNoiseOperator(min_sigma=0.0, max_sigma=0.0, random_state=0)
        result, params = op._apply(image)

        assert params["sigma"] == 0.0
        np.testing.assert_array_equal(result, image)

    def test_positive_sigma_changes_image(self, image: np.ndarray) -> None:
        """A strictly positive sigma changes at least some pixel values."""
        op = GaussianNoiseOperator(min_sigma=10.0, max_sigma=10.0, random_state=0)
        result, params = op._apply(image)

        assert params["sigma"] == 10.0
        assert not np.array_equal(result, image)

    def test_larger_sigma_increases_variance_of_change(
        self, image: np.ndarray
    ) -> None:
        """A larger sigma tends to produce a larger average pixel-wise change."""
        op_small = GaussianNoiseOperator(min_sigma=1.0, max_sigma=1.0, random_state=0)
        op_large = GaussianNoiseOperator(min_sigma=30.0, max_sigma=30.0, random_state=0)

        result_small, _ = op_small._apply(image)
        result_large, _ = op_large._apply(image)

        diff_small = np.abs(
            result_small.astype(np.float64) - image.astype(np.float64)
        ).mean()
        diff_large = np.abs(
            result_large.astype(np.float64) - image.astype(np.float64)
        ).mean()

        assert diff_large > diff_small


# ---------------------------------------------------------------------------
# Clipping
# ---------------------------------------------------------------------------


class TestClipping:
    """Tests for pixel value clipping into the valid [0, 255] range."""

    def test_clipping_at_high_end(self, bright_image: np.ndarray) -> None:
        """Large positive noise is clipped to 255, not overflowed."""
        op = GaussianNoiseOperator(min_sigma=80.0, max_sigma=80.0, random_state=0)
        result, _ = op._apply(bright_image)

        assert result.max() == 255
        assert result.dtype == np.uint8

    def test_clipping_at_low_end(self, dark_image: np.ndarray) -> None:
        """Large negative noise is clipped to 0, not underflowed."""
        op = GaussianNoiseOperator(min_sigma=80.0, max_sigma=80.0, random_state=0)
        result, _ = op._apply(dark_image)

        assert result.min() == 0
        assert result.dtype == np.uint8

    def test_clipping_never_produces_values_outside_valid_range(
        self, image: np.ndarray
    ) -> None:
        """Output pixel values always remain within [0, 255] regardless of sigma."""
        op = GaussianNoiseOperator(min_sigma=0.0, max_sigma=100.0, random_state=3)

        for _ in range(20):
            result, _ = op._apply(image)
            assert result.min() >= 0
            assert result.max() <= 255


# ---------------------------------------------------------------------------
# Probability and enabled gating
# ---------------------------------------------------------------------------


class TestGating:
    """Tests for probability=0 and enabled=False gating behaviour."""

    def test_probability_zero_never_applies(self, image: np.ndarray) -> None:
        """A probability of 0.0 means the operator is never applied."""
        op = GaussianNoiseOperator(probability=0.0, random_state=123)

        for _ in range(20):
            result = op.apply(image)
            assert result.applied is False
            assert result.success is True
            np.testing.assert_array_equal(result.image, image)

    def test_enabled_false_never_applies(self, image: np.ndarray) -> None:
        """A disabled operator never applies, regardless of probability."""
        op = GaussianNoiseOperator(probability=1.0, enabled=False, random_state=123)

        result = op.apply(image)

        assert result.applied is False
        assert result.success is True
        assert result.parameters == {}
        np.testing.assert_array_equal(result.image, image)

    def test_probability_one_always_applies(self, image: np.ndarray) -> None:
        """A probability of 1.0 means the operator always applies."""
        op = GaussianNoiseOperator(probability=1.0, random_state=123)

        for _ in range(20):
            result = op.apply(image)
            assert result.applied is True
            assert result.success is True


# ---------------------------------------------------------------------------
# AugmentationResult contents
# ---------------------------------------------------------------------------


class TestAugmentationResultContents:
    """Tests for the contents of the returned AugmentationResult."""

    def test_successful_result_contents(self, image: np.ndarray) -> None:
        """A successful application populates all expected result fields."""
        op = GaussianNoiseOperator(probability=1.0, random_state=1)
        result = op.apply(image)

        assert isinstance(result, AugmentationResult)
        assert result.operator_name == "gaussian_noise"
        assert result.applied is True
        assert result.success is True
        assert "sigma" in result.parameters
        assert result.error_message is None

    def test_skipped_result_contents(self, image: np.ndarray) -> None:
        """A skipped (disabled) application populates result fields correctly."""
        op = GaussianNoiseOperator(enabled=False, random_state=1)
        result = op.apply(image)

        assert result.operator_name == "gaussian_noise"
        assert result.applied is False
        assert result.success is True
        assert result.parameters == {}
        assert result.error_message is None

    def test_custom_operator_name_is_reflected_in_result(
        self, image: np.ndarray
    ) -> None:
        """A custom operator_name is reflected in the returned result."""
        op = GaussianNoiseOperator(
            probability=1.0, random_state=1, operator_name="my_gaussian_noise"
        )
        result = op.apply(image)

        assert result.operator_name == "my_gaussian_noise"


# ---------------------------------------------------------------------------
# Shape and dtype preservation
# ---------------------------------------------------------------------------


class TestShapeAndDtypePreservation:
    """Tests confirming output shape and dtype match the input."""

    def test_output_shape_preserved(self, image: np.ndarray) -> None:
        """Output image shape matches the input image shape."""
        op = GaussianNoiseOperator(probability=1.0, random_state=2)
        result = op.apply(image)

        assert result.image.shape == image.shape

    def test_output_dtype_preserved(self, image: np.ndarray) -> None:
        """Output image dtype remains uint8."""
        op = GaussianNoiseOperator(probability=1.0, random_state=2)
        result = op.apply(image)

        assert result.image.dtype == np.uint8

    @pytest.mark.parametrize("height,width", [(1, 1), (10, 20), (100, 50)])
    def test_various_shapes_preserved(self, height: int, width: int) -> None:
        """Output shape is preserved across a variety of synthetic image sizes."""
        rng = np.random.default_rng(0)
        synthetic_image = rng.integers(
            0, 256, size=(height, width, 3), dtype=np.uint8
        )
        op = GaussianNoiseOperator(probability=1.0, random_state=0)

        result = op.apply(synthetic_image)

        assert result.image.shape == synthetic_image.shape
        assert result.image.dtype == np.uint8


# ---------------------------------------------------------------------------
# Registry compatibility
# ---------------------------------------------------------------------------


class TestRegistryCompatibility:
    """Tests confirming GaussianNoiseOperator integrates with OperatorRegistry."""

    def test_register_and_retrieve(self) -> None:
        """GaussianNoiseOperator can be registered and retrieved by name."""
        registry = OperatorRegistry()

        registry.register("gaussian_noise", GaussianNoiseOperator)

        assert registry.get("gaussian_noise") is GaussianNoiseOperator
        assert "gaussian_noise" in registry

    def test_instantiate_from_registry(self, image: np.ndarray) -> None:
        """An operator class retrieved from the registry can be instantiated and applied."""
        registry = OperatorRegistry()
        registry.register("gaussian_noise", GaussianNoiseOperator)

        operator_cls = registry.get("gaussian_noise")
        op = operator_cls(probability=1.0, random_state=42)
        result = op.apply(image)

        assert isinstance(result, AugmentationResult)
        assert result.applied is True